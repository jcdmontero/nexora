...
## Específico de Nexora (spatie teams + CSRF)
- Con teams activos, fijar el contexto antes de asignar roles: `app(PermissionRegistrar::class)->setPermissionsTeamId($tenantId)`.
- Al crear roles en tests, pasar `team_id` explícito (firstOrCreate no lo autoasigna).
- Para probar endpoints POST con sesión: `$this->startSession()` + enviar `'_token' => csrf_token()` (la app valida CSRF también en tests). Evitar `withoutMiddleware()` completo en flujos que usan `session()` (ej. login con `regenerate()`).
- El SuperAdmin no tiene rol en BD; su acceso se valida vía `Gate::before(is_superadmin)`.
