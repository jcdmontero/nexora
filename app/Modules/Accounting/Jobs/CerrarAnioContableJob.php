...
class CerrarAnioContableJob implements ShouldQueue
{
    ...
    public function handle(CierreAnualService $cierreService): void
    {
        ...
        if (!$user) {
            Log::error("User not found for annual closure", ['user_id' => $this->userId]);
            return;
        }

        // Según reglas de arquitectura, fijar el team para spatie (tenant_id)
        setPermissionsTeamId($this->tenantId);

        if (!$user->can('accounting:admin')) {
            Log::error("User lacks accounting:admin permission for annual closure", ['user_id' => $this->userId]);
            return;
        }
