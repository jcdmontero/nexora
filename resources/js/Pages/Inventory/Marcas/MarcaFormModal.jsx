...
export default function MarcaFormModal({ isOpen, onClose, marca = null }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (marca) {
      put(route('inventory.marcas.update', marca.id), {
        onSuccess: () => {
          reset()
          onClose()
        },
      ...
      post(route('inventory.marcas.store'), {
        onSuccess: () => {
          reset()
          onClose()
        },
      ...
      <div className="relative z-50 w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-lg sm:p-8 animate-in fade-in zoom-in-95 duration-200">
        <div className="mb-6">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Añade marcas o fabricantes al catálogo de productos.
          </p>
        </div>
          ...
          <div className="flex items-start space-x-3 rounded-md border border-border p-4">
            ...
            <div className="space-y-1 leading-none">
              ...
              <p className="text-xs text-muted-foreground">
                Permite asignar productos a esta marca.
              </p>
            </div>
          </div>
